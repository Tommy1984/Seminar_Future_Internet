...
Laufzeitunterschiede entstehen können, verwednet Wang et al. in \cite{WWMA10} 
...
\end{tabular}
\end{center}
\caption{Übersicht über die Netztopologien \cite{WWMA10}}
\end{table}
...
OSPF reconvergence (recon) \cite{WWMA10}, Failur-Carring-Packet (FCP) \cite{tD2_LCRA07}, Path Splicing 
(PathSplicing) \cite{tD2_MEFV08}, R3 und Flow-based optimal link detour routing (opt) \cite{WWMA10}.


\subsection{Simulation}
Abbildung \ref{tD2_USISPMeasurement} zeigt die in\cite{WWMA10} ermittelten 
...
\end{minipage}
\caption{Einzel-, Zwei- und zufällige Drei-Fehler Szenarien (US-ISP)\cite{WWMA10}}
...
\Abbildungf{!htb}{width= 0.50\textwidth}{tD2_SBC_ThreeFailure}{tD2_SBC_ThreeFailure}{Leistungsverhältnis für Drei-Fehler Szenarien (SBC) \cite{WWMA10}}


\subsection{Reale Implementierung}
Neben der Simulation des Verhaltens verschiedener Routing-Protokolle in 
verschiedenen Topologien wurde in \cite{WWMA10} auch die Leistungsfähigkeit von 
...
\end{tabular}
\end{center}
\caption{R3 Offline-Phase in Sekunden \cite{WWMA10}}
\end{table}
