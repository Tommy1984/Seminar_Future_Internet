...
Internet eingesetzten Protokolle Open Shortest Path First (OSPF) 
\cite{tD2_RFC2328} oder Intermediate System - Intermediate System (IS-IS) 
\cite{tD2_RFC1142}. Eine wichtige Eigenschaft in diesem Zusammenhang ist 
...
Ein weiterer, oft nicht berücksichtigter Aspekt ist die Fehlendem Möglichkeit, 
...
Fehlerszenarien. Bei einer Obergrenze von $F$ sich zeitlich Überlappenden 
Fehlern, entspricht die Anzahl an möglichen Fehlerszenarien 
$\sum_{i=1}^{F} {|E| \choose i}$. Die Berücksichtigung jedes dieser Szenarien 
...
Routing Resiliency Reconfiguration (R3) ist nun ein neues Verfahren, um sowohl 
ein Routing für den Normalbetrieb ohne Fehler, als auch ein Protection Routing 
für den Fall eines oder mehrere Link Ausfälle zu berechnen und ständig auf die 
momentane Netztopologie anzupassen. Dazu wird ein Ansatz vorgestellt, 
der die Berücksichtigung aller möglichen Fehlerszenarien vermeidet.
Weiterhin wird gezeigt, dass das Verfahren Staufreiheit durch Link-Ausfälle 
garantiert. Zudem ist das Verfahren effizient in Hinsicht auf 
Berechnungs-Overhead und Speicherverbrauch, kann flexibel an verschieden 
Leistungsanforderungen (z.B zur Einhaltung von Service Level Agreements) 
angepasst werden und ist robust gegenüber Änderungen der Netzauslastung oder 
der Netztopologie.
...
als Fluss aufzufassen \cite{tD2_ApCo03} und das zugrundeliegende Computernetzwerk als 
Flussnetzwerk zu betrachten.
...
wird.
Die zweite Bedingun forde, dass der gesamte von einer Quelle initierte Verkehr 
weitergeleitet wird, während die dritte Bedingung verbietet, dass ein Teil 
davon wieder zur Quelle zurückgesendet wird.
Die vierte Bedinung fordert das der weitergeleitete Anteil zwischen $0\%$ und 
$100\%$ liegt.
...
von R3 verwendet wird ist das Multiprotolkol-Label-Switching (MPSL) 
...
Während beim klassichen Routing auf Schicht 3 jeder Router aufs neue den 
...
herangezogen. 
Ist der nächste Router der aktuelle Router, muss das Label wieder Stack 
entfernt werden.
...
Solgange die Urspüngliche Netztopologie besteht, kann das reguläre Routing verwendet werden. 
Bei Link-Ausfällen ist dieses Routing nicht mehr gültig und muss angepasst werden. 
...
Dadurch kann Ausfallzeit und damit die Verzögerung auf die Erkennung des 
Fehlers am lokalen Router und das aktivieren der Alterniv-Route beschränkt 
werden. 
...
dass es aufgrund ausgefallern Links und dadurch umgeleiten Netzwerkverkehr 
...
\end{Def}
Mit R3 hat Wang et al. nun ein Ansatz vorgestellt, 
der es ermöglicht, dieses Problem zu lösen.
