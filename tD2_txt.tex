...
$G'=(V, E \backslash \{e\})$ den vormals über diesem Link 
transportierten Verkehr zusätzlich übertragen.
Diese Zusatzbelastung hängt von der aktuellen Auslastung des ausgefallenen 
Links ab, kann aber höchsten so groß wie die Kapazität $c_{e}$ dieses Links sein 
und wird als \textit{Virtual Demand} bezeichnet. 
Ordnet man jedem Link $e \in E$ einen solchen Virtual Demand entsprechend 
seiner Kapazität zu, dann deckt die Konvexkombination aller Virtual Demands 
den gesamten umzuleitenden Verkehr ab.

Ordnet man jedem Link $e \in E$ einen Variable $x_{e}$ für den Virtual Demand 
des Links zu und erlaubt man bis zu $F$ fehlerhafte Links, dann überdeckt die 
Menge 
\begin{equation}
...
fehlerhaften Links enthält $X_{F}$ den gesamten umzuleitenden Verkehr.
Anstelle eines Routings für eine konstante Verkehrsmatrix $d$ bei variabler 
Netztopologie zu berechnen, kann nun ein Routing für die Originaltopologie bei 
...
Zu beachten ist hierbei, dass im Gegensatz zur endlichen Anzahl möglicher 
Netztopologien die Anzahl möglicher neuer Verkehrsmatrizen $d+X_{F}$ leider 
unendlich ist. 
...
Das explizite Aufzählen der unterschiedlichen Verkehrsmatrizen ist nicht 
notwendig.
...
Für ein genau Beschreibung der Transformation zwischen \ref{tD2_LPOriginal} 
und \ref{tD2_LPSimple} siehe \cite{tD2_WWMA10}
...
Dazu kann einfach der vormals an $e$ zugeteilte Verkehr $r_{ab}(e)$ 
...
Eine Shared Risk Link Group besteht aus Links die immer gleichzeitig 
unterbrochen werden.
Eine MLG besteht aus Links die vom Netzbetreiber zu Wartungszwecken zusammen 
abgeschaltet werden.
...
wird das Lineare Programm \ref{tD2_LPSimple} um zusätzliche Bedingungen ergänzt, 
mit denen der Abstand der Normalleistung vom Optimum beeinflusst werden kann.  
...
und zum Anderen das Verhältnis aus Worst-Case Verkehrsdichte und optimalem 
